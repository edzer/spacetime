...
\begin{document}
%\VignetteIndexEntry{ubsetting of spacetime objects}
\maketitle

\begin{abstract}
This vignette briefly describes how the subsetting of ST\dots classes 
works. The underlying assumptions and pitfalls to be aware of.
\end{abstract}
...
# data slot
m = 1:5*10 # means for each of the 3 point locations
mydata = data.frame(smpl1=rnorm(length(spObj)*length(timeObj),
                                mean=rep(m, 2)),
                    smpl2=rnorm(length(spObj)*length(timeObj),
                                mean=rep(m, 2)))
mydata[c(2, 4, 6, 7),] <- NA
...
Subsetting over space can be done in different ways. This includes \code{numeric} where we refer to each actual index of the spatial location, \code{logical} that must be of the same length as the spatial component; \code{character} referring to row names of the spatial object and \code{Spatial} initiating an overlay to retrieve selection indices. 

\subsection{Indices of class \code{numeric}}

Objects of type \emph{full} represent the complete structure of 6 spatio-temporal locations:
<<>>=
STFobj[1:3,]
@
...
The \emph{irregular} case stores for each spatio-temporal location a separate spatial and temporal index. Hence the same selection as above returns the first three entries, i.e. th first three locations at the same moment in time.
<<>>=
STIobj[1:3,]
@
...
Objects of type \emph{full} and \emph{sparse} require a vector of the same length as the slot \code{sp}. The vector will be recycled if necessary. The \emph{full} structure refers to 6 spatio-temporal locations
<<>>=
...
The \emph{irregular} case will again provide the exact matches with observation, but needs a longer vector of size six.
<<>>=
STSobj[c(TRUE,TRUE,FALSE,FALSE,TRUE,FALSE),]
@

A repeated selection or changing the order is obviously not possible for logical indices.
...
Objects of type \emph{full} represent the complete structure of 4 spatio-temporal locations:
<<>>=
STFobj[c("A","B"),]
@
...
Multiple selection and re-ordering of only the spatial domain are possible.
