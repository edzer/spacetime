...
cs = cshp()
names(cs)
row.names(cs) = paste(as.character(cs$CNTRY_NAME), seq_along(cs$CNTRY_NAME))
@
...
In the following fragment, we dselect missing dates and create the time index:
<<>>=
cs = cs[cs$COWSYEAR > -1,]
...
@
which turns out to be the {\em Germany Federal Republic} and {\em
Germany}, that is, before and after the merge. No data before
1955-5-5 is available.
