...
Select the first two \lstinline{Tracks} objects of a \lstinline{TracksCollection},
return an object of class \lstinline{TracksCollection}:
<<eval=TRUE, echo=TRUE, results=hide>>=
class(Tr[1:2])
dim(Tr[1:2])
@
Select the second \lstinline{Tracks} object of a tracks collection. Returns an
object of class \lstinline{Tracks}:
<<eval=TRUE, echo=TRUE>>=
class(Tr[2])
dim(Tr[2])
@
Select the first track of the second \lstinline{Tracks} object of a 
\lstinline{TracksCollection}. Returns an object of class \lstinline{Track}:
<<eval=TRUE, echo=TRUE>>=
class(Tr[2][1])
dim(Tr[2][1])
@
Select tracks 1 and 2 of the first \lstinline{Tracks} object as well as track
2 of the second \lstinline{Tracks} object of a \lstinline{TracksCollection},
return an object of class \lstinline{TracksCollection}.
<<eval=TRUE, echo=TRUE>>=
class(Tr[list(1:2, 2)])
dim(Tr[list(1:2, 2)])
@
Select any tracks of a tracks collection that intersect \lstinline{Spatial} 
object \lstinline{Muenster}.
<<eval=FALSE, echo=TRUE>>=
Tr[Muenster]
@
Select attribute \lstinline{co2} of a \lstinline{TracksCollection}, either by 
<<eval=TRUE, echo=TRUE>>=
class(Tr[["co2"]])
length(Tr[["co2"]])
@
